...
public class Logging1B {
    ...
    public void testBondsTables() throws IOException {
        ...
        try {
            ...
            // Locate the first link in table 1, column 1 ("Name")
            logger.info("Locating the first link in table 1, column 1 (Name).");
            test.info("Locating the first link in table 1, column 1 (Name).");
            WebElement firstNameLink = driver.findElement(By.xpath("(//table)[1]//tr[td[1]//a][1]/td[1]//a"));
            String firstNameHref = firstNameLink.getAttribute("href");
            logger.info("Opening the first 'Name' link in a new tab: " + firstNameHref);
            test.info("Opening the first 'Name' link in a new tab: " + firstNameHref);

            // Open the link in a new tab using JavaScript
            ((JavascriptExecutor) driver).executeScript("window.open(arguments[0], '_blank');", firstNameHref);

            // Switch to the new tab
            java.util.ArrayList<String> tabs = new java.util.ArrayList<>(driver.getWindowHandles());
            driver.switchTo().window(tabs.get(1));

            // Wait for the new page to load completely (up to 60 seconds)
            logger.info("Waiting for the new tab to load.");
            test.info("Waiting for the new tab to load.");
            new WebDriverWait(driver, Duration.ofSeconds(60)).until(
                webDriver -> ((JavascriptExecutor) webDriver)
                    .executeScript("return document.readyState").equals("complete"));

            // Take a screenshot in the new tab
            String screenshot5 = captureScreenshot(driver);
            test.addScreenCaptureFromPath(screenshot5);
            logger.info("Screenshot taken in the new tab.");
            test.info("Screenshot taken in the new tab.");
